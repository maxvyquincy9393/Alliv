    ...
    <FullScreenLayout>
      <div className="shell-content space-y-6 pb-12">
        <section className="panel flex flex-col gap-4 p-5 sm:p-6 md:flex-row md:items-center md:justify-between">
          ...
          {mainPhoto && (
            <div className="w-full md:w-1/2 rounded-[24px] overflow-hidden border border-white/10 shadow-[0_20px_35px_rgba(2,6,23,0.45)]">
        ...
        {secondaryPhotos.length > 0 && (
          <section className="grid grid-cols-2 gap-3 md:grid-cols-4">
            {secondaryPhotos.map((photo, index) => (
              <div
                key={photo + index}
                className="aspect-[4/5] rounded-[20px] overflow-hidden border border-white/10 bg-white/5"
              >
